...
func (c *Cmd) PushFavorites(configPath string) func(cCtx *cli.Context) error {
		...
		if cCtx.App.Metadata["useFavoritesAPI"].(bool) {
			...
			if len(result.LocalOnly) == 0 && len(result.Conflicts) == 0 {
				if len(c.config.Favorites) == len(apiFavorites) {
					...
					return c.DeleteLocalFavorites(configPath)(cCtx)
				}
...
func (c *Cmd) DeleteLocalFavorites(configPath string) func(cCtx *cli.Context) error {
	return func(cCtx *cli.Context) error {
